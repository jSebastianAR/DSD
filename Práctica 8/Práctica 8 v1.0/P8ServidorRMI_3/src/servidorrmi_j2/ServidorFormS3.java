 ...
 * @author jSeba
 */
public class ServidorFormS3 extends javax.swing.JFrame implements Runnable{
    ...
    //datos para server 2
    public ServidorFormS3() throws RemoteException {
        initComponents();
        ...
        try {
            ...
            server.servidor1Obj.agregaServer3(server);//agregamos el server3 a la referencia del server 1
        ...
        try {
            ...
            server.servidor2Obj.agregaServer3(server);//agregamos el server3 a la referencia del server 1
        } catch (NotBoundException ex) {
        ...
        try {
            ...
            Logger.getLogger(ServidorFormS3.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
                    ...
                    }catch(RemoteException e)
                    {
                        try 
                        {
                            if(server.servidor2Obj.comparaID(server.ID)==true)//hace la comparacion con el id del server 3
                            {
                                ...
                                server.servidor2Obj.soyCoordinador("localhost", server.ID);
                                agregaFES2();
                                actualizaFE(0);
                                actualizaFE(1);
                            }
