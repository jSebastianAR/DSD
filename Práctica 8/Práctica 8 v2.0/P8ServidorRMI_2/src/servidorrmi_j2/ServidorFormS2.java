...
public class ServidorFormS2 extends javax.swing.JFrame implements Runnable{
    ...
    public void obtenerReferenciaS3()
    {
        try {
            ...
            server.servidor3Obj.agregaServer2(server);//agregamos el server2 a la referencia del server 1
        } catch (NotBoundException | RemoteException ex) {
            System.out.println("No se pudo obtener la referencia del servidor 3 en servidor 2");
            return;
        }
    }
    ...
    public void actualizaFE(int i)//actualizara los datos del cooordinador en cada FE
    {
        try
        {
            server.FE[i].cambioCoordinador(server2Address, server.PUERTO, server.serverPass);//avisa a los jugadores la direccion del nuevo coordinador
        }catch(RemoteException | NullPointerException e)
        {
    ...
    public void run() {
        ...
        while(ct==h2)//hilo para las peticiones del cliente
        {
            ...
            {    
                ...
                {
                    System.out.println("Partida termino S2");
                    ...
                    }catch(RemoteException | NullPointerException e)
                    {
                        try 
                        {
                            if(server.servidor3Obj.comparaID(server.ID)==true)//hace la comparacion con el id del server 3
                            {
                                ...
                                server.servidor3Obj.soyCoordinador(server2Address, server.ID);
                                //agrega los FE al resto de servers
                                agregaFES3();
                                //actualizaFE(0);
                                //actualizaFE(1);
                            }
