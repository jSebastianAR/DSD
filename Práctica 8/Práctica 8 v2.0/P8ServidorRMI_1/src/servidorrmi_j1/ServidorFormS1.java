...
public class ServidorFormS1 extends javax.swing.JFrame implements Runnable{
    ...
    //public String server2Address = "10.100.73.173";//ip del servidor 2
    public String server2Address = "localhost";//ip del servidor 2
    public int server2Port = 4000;//puerto del server 1
    public String server3Address = "localhost";//ip del servidor 3
    public int server3Port = 5000;//puerto del server 3
    public int bool = 0;
    ...
    public ServidorFormS1() throws RemoteException {
        ...
        //OBTENIENDO LA REFERENCIA DEL SERVER 2
        obtenerReferenciaS2();
        //OBTENIENDO LA REFERENCIA DEL SERVER 2
    ...
    public void obtenerReferenciaS2()
    {
        try {
            ...
            server.servidor2Obj.agregaServer1(server);//agregamos el server2 a la referencia del server 1
    ...
    public void obtenerReferenciaS3()
    {
        try {
            Registry registry = LocateRegistry.getRegistry(server3Address, server3Port); //obtiene el registro de la maquina con la ip seleccionada
            server.servidor3Obj = (InterfaceServidores)(registry.lookup("server3"));//localiza lo guardado en el registro con la palabra "operacionservidor"
            server.servidor3Obj.agregaServer1(server);//agregamos el server2 a la referencia del server 1
        } catch (NotBoundException | RemoteException ex) {
            System.out.println("No se pudo obtener la referencia del servidor 3 en servidor 1");
            return;
        }
    }
    ...
    public void agregaFES2()
    {
        try
        {
            System.out.println("Agregue FES2");
    ...
    public void actualizaFE(int i)//actualizara los datos del cooordinador en cada FE
    {
        try
        {
            server.FE[i].cambioCoordinador(server1Address, server.PUERTO, server.serverPass);//avisa a los jugadores la direccion del nuevo coordinador
        }catch(RemoteException | NullPointerException e)
        {
            ...
            switch(server.id_coordinador)
            {
                case 1:
                    try{
                        ...
                        server.servidor3Obj.soyCoordinador(server1Address, server.ID);
                    //    actualizaFE(0);
                    //    actualizaFE(1);
                    }catch(RemoteException | NullPointerException e)
                    {
                ...
                case 2://si el coordinador es el server 1
                    ...
                    }catch(RemoteException | NullPointerException e)
                    {
                        ...
                        }catch (RemoteException | NullPointerException ex) //sino responde
                        {
                             //sino responde
